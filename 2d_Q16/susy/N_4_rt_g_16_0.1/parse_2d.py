...
#  print infile  # Monitor running status
  ...
  for line in open(infile):
    ...
    # Normalize using volume and Nc extracted above
      ...
      if fermAct[0] < 0:
        fermAct[0] = float((line.split())[8]) / (4.0 * vol * DIMF)  # edited fermAct [8] not in pure gauge  at [8] it may be det act
      elif fermAct[1] < 0:
      ...
      dS = (line.split())[4]              # edited 5th element of space seprated string
    ...
    elif line.startswith('MONITOR_FORCE_GAUGE '):   # edited monitor scalar force also
    ...
    # First four need Nc and DIMF extracted above
      ...
      temp = line.split()
      print >> PLAQ, "%g,%g,%g" % (MDTU, float(temp[4]) / Nc , float(temp[5]) / Nc) #edited temp[4] = 5th element of line/2 = ss_plaq include st = temp[5]plaquett here also same file
      print >> CG_ITERS, "%g,%g" % (traj, float(temp[3]))
      sB = float(temp[6]) / (1.5 * DIMF)           # edited sB=temp[6]
    ...
    # Wilson line in spatial direction
      ...
      x_r = float(temp[1]) / Nc
      x_i = float(temp[2]) / Nc
      print >> LINES, "%g,%g" % (x_r, x_i)  # edited Wilson line in spatial temp[1,2], temporal direction = temp[3,4] = polyakov loop
    ...
    # Unitarized Polyakov loop and spatial Wilson line
      ...
      print >> POLY_POLAR, "%g,%g" % (poly_r, poly_i)  #edited polar projected wilson line temp[3,4] =temporal
      poly_mod = math.sqrt(poly_r**2 + poly_i**2)      #edited polar projected wilson line temp[3,4] =temporal =  Unitarized Polyakov loop
      print >> POLY_MOD_POLAR, "%g,%g,%g,%g" % (MDTU, poly_mod, poly_r, poly_i)

      x_r = float(temp[1]) / Nc
      x_i = float(temp[2]) / Nc
      print >> LINES_POLAR, "%g,%g" % (x_r, x_i)         #edited polar projected wilson line temp[1,2] =spatial ,temp[5,6] = 3rd direction
      x_mod = math.sqrt(x_r**2 + x_i**2)                 #edited polar projected spatial wilson line temp[1,2]  =  Unitarized spatial Wilson line
      print >> LINES_MOD_POLAR, "%g,%g" % (MDTU, x_mod)

      #edited uniterized 3rd direction wilson line
    # ------------------------------------------------------------
